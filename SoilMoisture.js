...
var cor2 = [-87.6298, 41.8781];  // Downtown Chicago
...
var monthly = temporal_collection(sentinel, '2023', 12, 1, 'month')
  .filter(ee.Filter.listContains('system:band_names', 'B4'));  // ensures image has real bands
...
var vw = ee.Number(str_full_cover.max().reduceRegion({
  reducer: ee.Reducer.max(), geometry: geometry1, scale: 1000
  }).values().get(0));
  
var vd = ee.Number(str_full_cover.min().reduceRegion({
  reducer: ee.Reducer.min(), geometry: geometry1, scale: 1000
  }).values().get(0));
...
print(
  ui.Chart.image.series(sm, point, ee.Reducer.first(), 1000, 'system:time_start')
  )
  
Map.addLayer(sm.toBands().clip(geometry1),[],'sm',false);

Export.image.toDrive({
  image: sm.toBands().clip(geometry1),
  description: 'sen_sm',
  scale: 1000, 
  region: geometry1,
  maxPixels:  1e13,
